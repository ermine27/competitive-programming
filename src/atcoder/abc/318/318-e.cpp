...
int main() {
    ...
    fore(key, v, m) {
        ...
        rep(t, 1, nx) {
            // Ai=Ak=x かつ Ai!=Aj となる要素の個数を求める
            //   i は v[0]からv[t-1]までのt個
            //   k は v[k]からv[nx-1]までのnx-t個
            //   j は v[t-1]からv[t]までの間のx以外の要素 => v[t]-v[t-1]-1個
            // よって、この3つを掛け合わせたものが選び方の合計になる
            ans += t * (v[t] - v[t - 1] - 1) * (nx - t);
        }
    }
